...
export class ArticlesEditComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private message: NzMessageService, private route: ActivatedRoute) {
    ...
    this.validateForm = this.fb.group({
      ...
      subtitle: ['', [required, maxLength(50)]],
  ...
  submitForm(): void {
    if (this.validateForm.valid) {
      if (!this.articleId) {
        this.http
          .post('/article/blogarticle/create', { ...this.validateForm.value, tags: this.validateForm.value.tags.join(',') })
          .subscribe((res: any) => {
            if (res && res.success) {
        ...
        this.http
          .post('/article/blogarticle/update', {
            id: this.articleId,
            ...this.validateForm.value,
            tags: this.validateForm.value.tags.join(',')
          })
          .subscribe((res: any) => {
            if (res && res.success) {
  ...
  // 获取详情数据
  getArticleDetail() {
    this.http.get(`/article/blogarticle/detail/${this.articleId}`).subscribe((res: any) => {
      if (res && res.success) {
